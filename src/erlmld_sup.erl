...
init([Regname,
    ...
    {ok, PropertiesPathname} = erlmld_runner:build_properties(maps:put(port,
                                                                       ActualPort,
                                                                       Opts)),
    ...
    WorkerSup = #{id => wrk_sup,
                  ...
                  start =>
                      {erlmld_wrk_sup,
                       start_link,
                       [WorkerSupName, RecordProcessor, RecordProcessorData]}},
    ...
    TcpAcceptor = #{id => tcp_acceptor,
                    ...
                    start =>
                        {erlmld_tcp_acceptor,
                         start_link,
                         [AcceptorName, ListenSocket, StartWorker]}},

    MLDRunner = #{id => mld_runner,
                  type => worker,
                  shutdown => brutal_kill,
                  start =>
                      {erlmld_runner, start_link, [RunnerName, PropertiesPathname, StreamType]}},
